...
// @Tags         WhatsappIntegration
// @Produce      json
// @Param        payload	body 		requests.ProjectUpdateRequest	true 	"whatsapp integration update payload"
// @Success      200 		{object}	responses.Ok[entities.WhatsappIntegration]
// @Failure      400		{object}	responses.BadRequest
// @Failure 	 401    	{object}	responses.Unauthorized
// @Failure      422		{object}	responses.UnprocessableEntity
...
func (h *WhatsappIntegrationHandler) delete(c *fiber.Ctx) error {
	...
	err := h.service.Delete(ctx, &services.WhatsappIntegrationDeleteParams{
	...
	if stacktrace.GetCode(err) == repositories.ErrCodeNotFound {
		msg := fmt.Sprintf("cannot find integration with id [%s] for user [%s]", integrationID, authUser.ID)
		ctxLogger.Warn(stacktrace.Propagate(err, msg))
		return h.responseNotFound(c, msg)
	}
